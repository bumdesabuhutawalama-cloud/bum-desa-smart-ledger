...
export const Route = createFileRoute('/jasa/kas-bank')({
  component: KasBankJasa,
})

function KasBankJasa() {
  const { unitIdFilter, isConsolidating } = useUnitFilter()
  const navigate = useNavigate()

  // Query kas dan bank jasa dengan auto filter per unit
  const { data: kasBank, isLoading } = useQuery({
    queryKey: ['kas-bank-jasa', unitIdFilter],
    queryFn: async () => {
  ...
  return (
    <UnitLayout
      title="Kas & Bank Jasa"
      description="Kelola transaksi kas dan rekening bank unit jasa & sewa"
      unitKode="JASA"
    >
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Transaksi kas dan bank unit jasa</p>
        </div>
        <Button onClick={() => navigate({ to: '/jasa/kas-bank/baru' })}>
